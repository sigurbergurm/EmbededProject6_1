...
//When pressing enter it outputs "Hello from process XXXX". At the start there is no process and therefor writes just empty string
//the 2nd time its ran then it outputs the processor id as well. And then when it is run consecutively it overwrites that data and writes
//the new process id